...
const demo = [
  {public_no:"demo1",img:"./assets/work1.svg",city:"Москва",style:"Реализм",work_price:20000,title:"Черно-серый реализм · полный день",description:"Демонстрационная карточка интерфейса.",master_name:"Демо-мастер",plan:"top",isDemo:true},
  {public_no:"demo2",img:"./assets/work2.svg",city:"Санкт-Петербург",style:"Old School",work_price:15000,title:"Old School · предплечье",description:"Демонстрационная карточка интерфейса.",master_name:"Демо-мастер",plan:"basic",isDemo:true},
  {public_no:"demo3",img:"./assets/work3.svg",city:"Москва",style:"Графика",work_price:12000,title:"Графика · авторский эскиз",description:"Демонстрационная карточка интерфейса.",master_name:"Демо-мастер",plan:"basic",isDemo:true}
];
...
function card(x){
  ...
  const href = x.isDemo ? "#" : `./listing.html?n=${encodeURIComponent(x.public_no)}`;
...
function apply(){
  ...
  document.querySelector("#cards").innerHTML = list.map(card).join("") ||
...
async function load(){
  try{
    ...
    if(Array.isArray(live) && live.length){
      ...
      document.querySelector("#statText").textContent = "реальных опубликованных работ";
    } else {
      all = demo;
      document.querySelector("#liveCount").textContent = "ДЕМО";
      document.querySelector("#statText").textContent = "пока нет опубликованных работ";
    }
  } catch(e) {
    console.error(e);
    all = demo;
    document.querySelector("#liveCount").textContent = "ДЕМО";
    document.querySelector("#statText").textContent = "каталог временно недоступен";
  }
  apply();
}
